...
const ParametersSelectDesktop = ({title, setOption, currentOption, optionButtons, loading, isMobile = false}) => {
    ...
    return <div className="mt-5">
        ...
        {(!!loading && <Spin/>)}
        <Radio.Group prefixCls="app-vertical" size="large" onChange={onSetOption} value={currentOption}>
            {Options}
        </Radio.Group>
    </div>;
};

const ParametersSelectMobile = ({title, setOption, currentOption, optionButtons, loading, isMobile = false}) => {
    const Options = useMemo(() => optionButtons.map(
...
const ParameterSelect = props => props.isMobile
